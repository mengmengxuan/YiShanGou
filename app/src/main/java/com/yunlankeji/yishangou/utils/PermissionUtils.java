...
public class PermissionUtils {
    ...
    public static void checkMorePermissions(Context context, String[] permissions, PermissionCheckCallBack callBack) {
        List<String> permissionList = checkMorePermissions(context, permissions);//获取未授权的权限
        if (permissionList.size() == 0) {  // 用户已授予权限
            callBack.onHasPermission();
        } else {
            boolean isFirst = true;
            for (int i = 0; i < permissionList.size(); i++) {
                String permission = permissionList.get(i);//获取单个未授权的权限
                if (judgePermission(context, permission)) {//判断用户之前是否拒绝过此权限
                    isFirst = false;
                    break;
                }
            }
            ...
            else       // 用户之前已拒绝并勾选了不再询问、用户第一次申请权限。
